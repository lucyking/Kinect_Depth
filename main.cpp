...
struct sysinfo sys_info; //use sys-time as the picture squenence stamp
char uptimeInfo[15];
...
void glutDisplay (void)
{
	...
	for (XnUInt y = 0; y < g_depthMD.YRes(); ++y)             //   <-- Res ==> [Resolution]
	{
		...
		if( y == (g_depthMD.YRes()-1) && mytmp > 0 ){
			printf("%s>[%04d,%04d]:%.0lf\n",uptimeInfo,x_min,y_min,mytmp);//0.1236 * tan(minDepth / 2842.5 + 1.1863)
		}
